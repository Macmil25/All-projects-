    ...
    with open(filename) as f:
        ...
        for row in reader:
            ev = [
             ...
             visitor_to_int(row[15]),#visitor type
             weekend_to_int(row[16]) #weekend
        ]

        evidence.append(ev)
        labels.append(revenue_to_int(row[17])) #Revenue

    return evidence,labels

def month_to_int(month):
    """Convert month string to integer(Jan=0,Dec11)."""
    months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
    return months.index(month)

def visitor_to_int(visitor):
    """Convert vistortype to integer (1 if returning_visitor, else 0)."""
    return 1 if visitor == 'Returning_Visitor' else 0

def weekend_to_int(weekend):
    """Convert weekend to integer(1 if TRUE,else 0)."""
    return 1 if weekend == 'TRUE' else 0

def revenue_to_int(revenue):
    """Convert Revenue to integer(1 if TRUE, else 0)."""
